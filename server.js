 ...
 */
async function buildLoop(rawPath, loopPath, scale) {
  const filter = [
    '[0:v]split[a][b];',
...
async function makeVisual(job, { slug, aspect, prompt }) {
  ...
  await buildLoop(rawPath, loopPath, scale);
...
/**
 * Rebuild loops from raw clips already on disk, at the capped bitrate. Costs
 * nothing — no fal call — and is how you fix loops encoded before the cap
 * existed. Pass {"all": true} to redo every raw clip.
 */
app.post('/jobs/reloop', (req, res) => {
  const body = req.body || {};
  const job = startJob('reloop', { all: Boolean(body.all), slug: body.slug }, async (j) => {
    ...
    for (const slug of targets) {
      ...
      const scale = isWide ? '1920:1080' : '1080:1920';
      step(j, `re-encoding ${slug} at capped bitrate`);
      await buildLoop(
    ...
    return { rebuilt: rebuilt.length, slugs: rebuilt, disk: await diskUsage() };
  });
  res.status(202).json({ job_id: job.id, status: job.status });
});
